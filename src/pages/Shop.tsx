...
export const Shop = () => {
  ...
  const priceFilteredProducts = activePriceRange === 'all' ? categoryFilteredProducts : categoryFilteredProducts.filter(product => product.price >= priceRanges[activePriceRange as keyof typeof priceRanges][0] && product.price <= priceRanges[activePriceRange as keyof typeof priceRanges][1]);
  // Sort products
            ...
            <div className="lg:col-span-9">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 pb-4 border-b border-gray-200">
                <div className="mb-4 sm:mb-0">
                  ...
                  {(activeCategory !== 'all' || activePriceRange !== 'all') && <div className="mt-1 text-sm text-gray-500">
                      Filtered by:
                      {activeCategory !== 'all' && <span className="ml-1">{activeCategory}</span>}
                      {activePriceRange !== 'all' && activeCategory !== 'all' && <span>, </span>}
                      {activePriceRange !== 'all' && <span>
                          Price:{' '}
                          {activePriceRange === 'under-10' ? 'Under $10' : activePriceRange === 'over-50' ? 'Over $50' : `$${priceRanges[activePriceRange as keyof typeof priceRanges][0]} - $${priceRanges[activePriceRange as keyof typeof priceRanges][1]}`}
                        </span>}
                      <button onClick={() => {
                    setActiveCategory('all');
                    setActivePriceRange('all');
              ...
              {/* Products grid */}
              {currentProducts.length === 0 ? <div className="text-center py-12">
                ...
                </div> : <div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
